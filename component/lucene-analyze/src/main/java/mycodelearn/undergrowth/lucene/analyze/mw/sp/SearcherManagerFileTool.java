 ...
 * @version 1.0.0
 */
public class SearcherManagerFileTool {

	private Logger logger = LoggerFactory.getLogger(SearcherManagerFileTool.class);
	private String[] data = { "name", "path", "content", "size", "lastTime", "length", "id" };
	private SearcherManager searcherManager = null;

	public SearcherManagerFileTool(SearcherManager searcherManager) {
		...
		if (scoreDocs != null && scoreDocs.length > 0) {
			...
			for (ScoreDoc scoreDoc : scoreDocs) {
				...
				if (scoreDoc != null) {
					...
					logger.info(scoreDoc.doc + ":" + "[" + scoreDoc.score + "]" + data[0] + ":" + document.get(data[0])
							+ "\t" + data[1] + ":" + document.get(data[1]) + "\t" + data[2] + ":"
							+ document.get(data[2]) + "\t" + data[3] + ":" + document.get(data[3]) + "\t" + data[4]
