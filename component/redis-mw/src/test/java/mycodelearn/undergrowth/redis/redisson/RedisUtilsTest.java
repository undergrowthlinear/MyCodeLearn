...
public class RedisUtilsTest {
	...
	/**
	 * RBucket 映射为 redis server 的 string 类型 只能存放最后存储的一个字符串 redis server 命令:
	 * 查看所有键---->keys * 查看key的类型--->type testBucket 查看key的值 ---->get testBucket
	 */
	@Test
	...
	/**
	 * RMap 映射为 redis server 的 hash 类型 分为 put(返回键值) 、 fast(返回状态) 同步 异步 redis
	 * server 命令: 查看所有键---->keys * 查看key的类型--->type testMap 查看key的值 ---->hgetall
	 * testMap
	 * 
	...
	/**
	 * RSortedSet 映射为 redis server 的 list 类型 存储以有序集合的形式存放 redis server 命令:
	 * 查看所有键---->keys * 查看key的类型--->type testSortedSet 查看key的值 ---->lrange
	 * testSortedSet 0 10
	 */
	@Test
	...
	/**
	 * RSet 映射为 redis server 的set 类型 redis server 命令: 查看所有键---->keys *
	 * 查看key的类型--->type testSet 查看key的值 ---->smembers testSet
	 */
	@Test
	...
	/**
	 * RList 映射为 redis server的list类型 redis server 命令: 查看所有键---->keys *
	 * 查看key的类型--->type testList 查看key的值 ---->lrange testList 0 10
	 */
	@Test
	 ...
	 * 查看key的类型--->type testQueue 查看key的值 ---->lrange testQueue 0 10
	 */
	@Test
	...
	/**
	 * RDeque 映射为 redis server 的 list类型 双端队列--对头和队尾都可添加或者移除，也遵循队列的先入先出 redis
	 * server 命令: 查看所有键---->keys * 查看key的类型--->type testDeque 查看key的值
	 * ---->lrange testDeque 0 10
	 */
	@Test
	...
	/**
	 * RLock 映射为redis server的string 类型 string中存放 线程标示、线程计数 查看所有键---->keys *
	 * 查看key的类型--->type testLock1 查看key的值 ---->get testLock1 如果想在redis server中
	 * 看到 testLock1 就不能使用 rLock.unlock(); 因为使用 rLock.unlock(); 之后 就会删除redis
	 * server中的 testLock1
	 * 
	 */
	@Test
	...
	/**
	 * RAtomicLong 映射为redis server的string 类型 string中数值 查看所有键---->keys *
	 * 查看key的类型--->type testAtomicLong 查看key的值 ---->get testAtomicLong
	 */
	@Test
	...
	/**
	 * RCountDownLatch 映射为redis server的string 类型 string中数值 闭锁--等待其他线程中的操作都做完
	 * 在进行操作 查看所有键---->keys * 查看key的类型--->type testCountDownLatch 查看key的值
	 * ---->get testCountDownLatch
	 */
	@Test
