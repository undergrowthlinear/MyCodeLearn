package mylearncode.undergrowth.dubbo.curator;
...
public class CuratorLearn {
	...
	public void createNodeAsync(String path, String data) {
		...
		try {
			client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT)
					.inBackground(new CustomBackgrundCallbaxk(countDownLatch), executor).forPath(path, data.getBytes());
			client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT)
	...
	public boolean deleteNode(String path) {
		...
		try {
			...
			logger.info("stat:" + stat.getCtime() + "\t" + stat.getCversion() + "\t" + stat.getMzxid() + "\t"
					+ stat.getVersion());
	...
	public boolean setData(String path, String newData) {
		...
		try {
			client.getData().storingStatIn(stat).forPath(path);
			logger.info("stat:" + stat);
			logger.info("stat:" + client.setData().withVersion(stat.getVersion()).forPath(path, newData.getBytes()));
			logger.info("stat:" + client.setData().withVersion(stat.getVersion()).forPath(path, newData.getBytes()));
			return true;
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public boolean nodeCache(String path, int count, String newData) {
		...
		try {
			...
			nodeCache.getListenable().addListener(new CustomNodeCacheListener(nodeCache, countDownLatch), executor);
	...
	public boolean pathChildrenCache(String path, int count, String newData) {
		...
		try {
			...
			childrenCache.getListenable().addListener(new CustomChildrenCache(childrenCache, countDownLatch), executor);
			/*
	...
	public boolean distriCyclicBarrier(String connectString, String barrierPath, int memberQty) {
		...
		for (int i = 0; i < memberQty; i++) {
			RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3);
			CuratorFramework client = CuratorFrameworkFactory.newClient(connectString, 15000, 10000, retryPolicy);
			client.start();
			DistributedDoubleBarrier doubleBarrier = new DistributedDoubleBarrier(client, barrierPath, memberQty);
	...
	public boolean serverCluster(int num) {
		...
		try {
			...
			/*
			 * leader.kill(); for (TestingZooKeeperServer zooKeeperServer :
			 * cluster.getServers()) {
			 * logger.info(zooKeeperServer.getInstanceSpec().getServerId() + "-"
			 * + zooKeeperServer.getQuorumPeer().getQuorumSize() + "-" +
			 * zooKeeperServer.getInstanceSpec().getDataDirectory() + "-" +
			 * zooKeeperServer.getQuorumPeer().getServerState()); if
			 * ("leading".equals(zooKeeperServer.getQuorumPeer().getServerState(
			 * ))) { leader=zooKeeperServer; } }
			 */
			cluster.stop();
		} catch (Exception e) {
	...
	public boolean testingServer(String pathname, String path) {
		try {
			...
			RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3);
			client = CuratorFrameworkFactory.newClient(testingServer.getConnectString(), 15000, 10000, retryPolicy);
			client.start();
