	...
	class X {
		...
		public void m() {
			...
			try {
				...
				// ... method body
				Thread.sleep(1);
				System.out.println(System.currentTimeMillis()+",doSomeThing...,"+Thread.currentThread().getId() + "," + Thread.currentThread().getName());
		...
		public void m() {
			if (lock.tryLock()) {
				try {
					...
					System.out.println("获取到锁,"+Thread.currentThread().getId() + "," + Thread.currentThread().getName());
					//Thread.sleep(1);
				} /*catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				...
				System.out.println("没有获取到锁,"+Thread.currentThread().getId() + "," + Thread.currentThread().getName());
			}
		}
	}
		...
		public void m(int timeOut) throws InterruptedException {
			if (lock.tryLock(timeOut, TimeUnit.MILLISECONDS)) {
				try {
					// manipulate protected state
					System.out
							.println("获取到锁," + Thread.currentThread().getId() + "," + Thread.currentThread().getName());
				} finally {
					lock.unlock();
				}
			} else {
				// perform alternative actions
				System.out.println(timeOut+"ms后没有获取到锁," + Thread.currentThread().getId() + "," + Thread.currentThread().getName());
			}
