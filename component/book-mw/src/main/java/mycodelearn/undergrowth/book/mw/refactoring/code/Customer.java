...
public class Customer {
	...
	public String statement() {
		...
		for (Iterator<Rental> iterator = rentals.iterator(); iterator.hasNext();) {
			...
			// 计算影片详细信息
			body.append(rental.getMovie().getTitle() + " rental " + rental.getRentalDay() + " days, amount is "
					+ rental.getCharge() + "\t");
		}
		showDetail.setBody(body);
		showDetail.setTail(" all movie amount is " + getAllMount() + ",all movie frequencyRental is " + getFrequencyRental());
